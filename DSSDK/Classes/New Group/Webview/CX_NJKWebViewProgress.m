...
#import "CX_NJKWebViewProgress.h"

static NSString * const CX_completeRPCURL = @"webviewprogressproxy:///complete";

const CGFloat CX_NJKInitialProgressValue = 0.1;
const CGFloat CX_NJKInteractiveProgressValue = 0.5;
const CGFloat CX_NJKFinalProgressValue = 0.9;

@implementation CX_NJKWebViewProgress
{
...
- (void)startProgress
{
    if (_progress < CX_NJKInitialProgressValue) {
        [self setProgress:CX_NJKInitialProgressValue];
    }
}
...
- (void)incrementProgress
{
    ...
    CGFloat maxProgress = _interactive ? CX_NJKFinalProgressValue : CX_NJKInteractiveProgressValue;
...
{
    if ([request.URL.absoluteString isEqualToString:CX_completeRPCURL]) {
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if (interactive) {
        _interactive = YES;
        NSString *waitForCompleteJS = [NSString stringWithFormat:@"window.addEventListener('load',function() { var iframe = document.createElement('iframe'); iframe.style.display = 'none'; iframe.src = '%@'; document.body.appendChild(iframe);  }, false);", CX_completeRPCURL];
