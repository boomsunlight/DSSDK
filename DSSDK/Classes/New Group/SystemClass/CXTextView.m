...
- (void)layoutSubviews{
    ...
    if (_placeHolderLabel.hidden == NO && self.isAutoHeight) {
        CGFloat placeHolderHeight = _placeHolderLabel.frame.size.height + (_placeHolderLabelInsets.top + _placeHolderLabelInsets.bottom);
        if (placeHolderHeight > self.frame.size.height) {
//            self.frame.size.height = placeHolderHeight;
            self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, placeHolderHeight);
        }
    }
